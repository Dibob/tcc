...
class FisherFaces:
    def __init__(self):
        ...
        self.modelo_fisher_train = cv2.createFisherFaceRecognizer()
        self.pessoas = []
        self.face_names = []
    ...
    def carregar_dados(self):
        ...
        for (subdirs, dirs, files) in os.walk(self.diretorio_faces):
            for subdir in dirs:
                ...
                key += 1
        self.names = names 
        self.modelo_fisher_train.load('src/tcc_robotica/fisher_trained_data.xml')
    ...
    def processar_imagem(self, img):
	    ...
	    taxa_confianca = self.modelo_fisher_train.predict(face_resized)
	    
            if taxa_confianca[1]<300:
